...
const AiTraining = () => {
  ...
  const submitForm = async () => {
    ...
    const dataToSend = {
      type: sanitizeString(selectedOption),
      file_title: sanitizeString(formData.fileTitle),
      recipe: Number(formData.recipeId), // Ensure recipe is a number
      file: await getFileAsBase64(files[0]), // Convert file to base64 or null
    ...
    try {
      const response = await aiTraining({
        formDataToSend: dataToSend,
        id: Number(formData.recipeId), // Use recipeId in URL
      });
      ...
      <div className="rounded-lg p-6 px-14 lora">
        ...
        <div className="relative mb-8 px-6">
          <div className="flex justify-between relative">
            ...
            {/* Steps */}
            {[
              { step: 1, label: 'Select content type', image: img },
              { step: 2, label: 'Upload files', image: img },
              { step: 3, label: 'Add metadata', image: img },
              { step: 4, label: 'Review & confirm', image: img },
            ].map(({ step, label, image }) => (
              <div key={step} className="relative z-10 flex flex-col items-center">
                <div
                  className={`relative w-14 h-14 mx-auto mb-2 rounded-full ${
                    maxStepReached >= step ? 'bg-[#5B21BD]' : 'bg-gray-200'
                  }`}
                >
                  <img
                    src={image}
                    ...
                    className={`w-14 h-14 object-cover rounded-full ${
                      maxStepReached >= step ? 'opacity-100' : 'opacity-50'
                    }`}
                  />
                  <span
                ...
                <p
                  className={`text-sm font-medium ${
                    maxStepReached >= step ? 'text-[#5B21BD]' : 'text-gray-500'
                  }`}
                >
                  {label}
                </p>
            ...
            <div className="flex w-full gap-6">
              ...
              <div className="mb-4 md:w-1/2 relative">
                ...
                <Select
                  ...
                  value={
                    formData.recipeId && recipesData?.data
                      ? {
                          ...
                          label:
                            recipesData.data.find((recipe) => String(recipe.id) === String(formData.recipeId))?.title ||
                            'Select a recipe',
                        }
                      : null
                  }
                  ...
                  classNames={{
                    control: ({ isFocused }) =>
                      `h-[45px] border border-[#5B21BD] rounded-md ${
                        isFocused ? 'ring-2 ring-[#5B21BD] shadow-[0_0_0_2px_rgba(91,33,189,0.2)]' : ''
                      } p-2`,
                    menu: () =>
          ...
          <div className="mt-8 bg-white rounded-xl border border-[#EFE9F8] lora p-10">
            ...
            <div className="flex justify-between mt-8">
              ...
              <button
                ...
                onClick={submitForm}
                disabled={isLoading || !formData.recipeId || !formData.fileTitle}
              >
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-lg">
              ...
              {isSuccess && apiResponse?.data && (
                <div className="mb-6 text-sm text-gray-600">
                  ...
                  <p><strong>Recipe ID:</strong> {apiResponse.data.recipe || 'N/A'}</p>
